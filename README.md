...
It walks through one prerequisite plus six steps. Each step produces one source-of-truth document, finalized by a pull request. **Claude Code users** run the `/init-*` slash commands for an automated flow. **Teams using Copilot or any other AI assistant** follow the step guides in `docs/guides/proj-init/` directly in their AI chat.

## Prerequisites

- **Git** and a repository on a supported **Git host** — GitHub, Azure DevOps, Bitbucket, or GitLab. Step 0 configures branch governance to match your host, plan, and team size — see [Step 0](docs/guides/proj-init/00-repo-setup.md) for what's available on free vs. paid plans.
...
| ---- | ------- | -------- |
| 0 | *manual setup* | branch protection + `CONTRIBUTING.md` (governance); required-reviewer policy if plan supports it |
| 1 | `/init-product` | `PRODUCT.md` |
| 2 | `/init-prd` | `PRD.md` |
| 3 | `/init-architecture` | `ARCHITECTURE.md` |
| 4 | `/init-techstack` | `TECH-STACK.md` (+ `CONTRIBUTING.md` tooling layer) |
| 5 | `/init-aitools` | `AI-TOOL-GUIDE.md` + one adapter per AI tool in use (e.g. `CLAUDE.md`, `.github/copilot-instructions.md`) |
...
.claude/commands/                ← /init-* slash commands + /doc-update (Claude Code automation)
README.md                        ← this boilerplate entrypoint (replaced in Step 6)

Generated after running Steps 1–6:
PRODUCT.md                       ← product concept (Step 1)
PRD.md                           ← requirements (Step 2)
ARCHITECTURE.md                  ← system design (Step 3)
TECH-STACK.md                    ← approved technologies (Step 4)
CONTRIBUTING.md                  ← governance + tooling rules (Steps 0 and 4)
AI-TOOL-GUIDE.md                 ← AI tool rules shared across all tools (Step 5)
CLAUDE.md                        ← Claude Code adapter, if in use (Step 5)
.github/copilot-instructions.md  ← Copilot adapter, if in use (Step 5)
README.md                        ← project entry point, replaces this file (Step 6)
```
