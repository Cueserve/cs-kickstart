---
description: "Step 6: generate AI-TOOL-GUIDE.md + tool adapter files (Architect)"
allowed-tools: Read, Write, Edit, Glob, Bash(git:*), Bash(gh:*), Bash(az:*), Bash(glab:*)
---

# /init-aitoolguide — Step 6: AI Tool Guide

You are running **Step 6** of the Project Initiation process. This step produces **`AI-TOOL-GUIDE.md`** plus its tool adapter files. Owned by the **Architect**.
...
2. **Upstream document on `main`** — run `git show main:TECH-STACK.md`.
   - Exit 0 → proceed.
   - Non-zero → **STOP.** Tell the user: "TECH-STACK.md is not on `main` — Step 5 must be merged before Step 6 can run."
...
- From an up-to-date `main`, create and switch to `init/aitoolguide`.
- If `init/aitoolguide` already exists, do not reuse it silently — ask the user how to proceed.

## 3. Produce the documents

- Read the guide and follow it exactly: @docs/guides/proj-init/06-ai-tool-guide.md
...
- Commit `AI-TOOL-GUIDE.md` and the generated adapter files to `init/aitoolguide` with a Conventional Commit message (e.g. `docs: add AI-TOOL-GUIDE.md and tool adapters (Step 6)`).
...
- On confirmation: push `init/aitoolguide` and open a PR/MR targeting `main` via your host's CLI (`gh pr create` / `az repos pr create` / `glab mr create`) — or, if no CLI is available, push the branch and give the user the URL to open the PR/MR in the host's web UI. Title references Step 6; the body summarizes the AI rules, lists the adapter files generated, and includes the reviewer checklist from `docs/guides/proj-init/06-ai-tool-guide.md`.
