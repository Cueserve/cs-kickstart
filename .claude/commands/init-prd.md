---
description: "Step 2: generate PRD.md from its initiation guide (Product Owner)"
allowed-tools: Read, Write, Edit, Glob, Bash(git:*), Bash(gh:*), Bash(az:*), Bash(glab:*)
---

# /init-prd — Step 2: Product Requirements Document

You are running **Step 2** of the Project Initiation process. This step produces **`PRD.md`**, owned by the **Product Owner**.
...
2. **Upstream document on `main`** — run `git show main:PRODUCT.md`.
   - Exit 0 → proceed.
   - Non-zero → **STOP.** Tell the user: "PRODUCT.md is not on `main` — Step 1 must be merged before Step 2 can run."
...
- Read the guide and follow it exactly: @docs/guides/proj-init/02-prd.md
...
- Commit `PRD.md` to `init/prd` with a Conventional Commit message (e.g. `docs: add PRD.md (Step 2)`).
